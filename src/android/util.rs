...
{
    unsafe { Local::from_env_object(env.get_native_interface(), env.new_string(s).expect(&format!("Creating java string for '{:?}'", s)).into_inner()) }
}

pub fn java_string_array<'a>(env: &'a JNIEnv, size: jsize) -> Result<Local<'a, ObjectArray<JavaString, Throwable>>> {
    unsafe {
        let class = env.find_class("java/lang/String").map_err(|e| e.description().to_string())?;
        let object = env.new_object_array(size, class, JObject::null()).map_err(|e| e.description().to_string())?;
        let exception = env.exception_occurred().map_err(|e| e.description().to_string())?;
...
pub fn java_key_generator<'a>(env: &'a JNIEnv, algorithm: &'a JavaString, provider: &'a JavaString) -> Result<Local<'a, KeyGenerator>> {
...
pub fn java_algorithm_parameter_spec<'a>(
    ...
    let x: Local<'a, KeyGenParameterSpec_Builder> = stringify_throwable!(KeyGenParameterSpec_Builder::new(
        unsafe { jni_glue::Env::from_ptr(env.get_native_interface()) },
        alias,
...
pub fn java_algorithm_parameter_spec_from_bytes<'a>(env: &'a JNIEnv, iv: &'a ByteArray) -> Result<Local<'a, AlgorithmParameterSpec>> {
    let spec = stringify_throwable!(IvParameterSpec::new_byte_array(unsafe { jni_glue::Env::from_ptr(env.get_native_interface()) }, Some(iv)))?;
    Ok(unsafe { std::mem::transmute::<Local<'_, IvParameterSpec>, Local<'_, AlgorithmParameterSpec>>(spec) })
}
...
pub fn java_cipher<'a>(env: &'a JNIEnv, transform: &'a JavaString, mode: i32, secret_key: Local<'a, Key>, spec: Option<&'a AlgorithmParameterSpec>) -> Result<Local<'a, Cipher>> {
    let cipher = r#try!(resopt!(Cipher::getInstance_String(unsafe { jni_glue::Env::from_ptr(env.get_native_interface()) }, transform)));
    let _ = stringify_throwable!(cipher.init_int_Key_AlgorithmParameterSpec(mode, Some(&*secret_key), spec))?;
    Ok(cipher)
}

pub fn java_base64_encode<'a>(env: &'a JNIEnv, bytes: &'a ByteArray) -> Result<Local<'a, JavaString>> {
...
pub fn java_base64_decode<'a>(env: &'a JNIEnv, s: &'a JavaString) -> Result<Local<'a, ByteArray>> {
    resopt!(Base64::decode_String_int(unsafe { jni_glue::Env::from_ptr(env.get_native_interface()) }, Some(s), Base64::DEFAULT))
}
...
pub fn java_keystore<'a>(env: &'a JNIEnv, provider: &'a JavaString) -> Result<Local<'a, KeyStore>> {
    resopt!(KeyStore::getInstance_String(unsafe { jni_glue::Env::from_ptr(env.get_native_interface()) }, provider))
}
