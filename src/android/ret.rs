...
impl<'a, Inner: Return<'a, Env = JNIEnv<'a>> + Default> Return<'a> for Option<Inner> {
    ...
    fn convert(env: &Self::Env, val: Self) -> Self::Ext {
        ...
        match val {
            ...
            None => {
                ...
                let exception: JThrowable<'a> = env.new_object(class, "()V", &[]).expect("Must be able to instantiate the Exception; qed").into();
                // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                // !!!!                                                        !!!!
    ...
    fn convert_without_exception(env: &Self::Env, val: Self) -> Self::Ext {
        match val {
            ...
            None => {
                Return::convert_without_exception(env, Inner::default())
            }
        }
    }
}
